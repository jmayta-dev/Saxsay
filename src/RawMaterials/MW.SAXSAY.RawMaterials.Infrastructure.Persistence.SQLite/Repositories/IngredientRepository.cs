...
public class RawMaterialRepository : Repository, IRawMaterialRepository
{
    ...
    #region Methods
    public Task<RawMaterial> GetAsync(CancellationToken cancellationToken = default)
    {
        throw new NotImplementedException();
    }

    public Task<RawMaterial> GetBySpecificationAsync(
    ...
    {
        List<RawMaterial> ingredients = new();
        string query =
        @"
        ...
        if (reader.HasRows)
        {
            ...
            {
                RawMaterial.RawMaterialBuilder ingredientBuilder = new();
                var ingredientId = new RawMaterialId(reader.GetInt64(reader.GetOrdinal("Id")));
    ...
    public async Task<RawMaterialId?> RegisterAsync(
        RawMaterial ingredient, CancellationToken cancellationToken = default)
    {
        RawMaterialId? newIngredientId = default;
        string query =
        @"
        ...
        if (long.TryParse(result?.ToString(), out long newId))
            newIngredientId = new RawMaterialId(newId);
